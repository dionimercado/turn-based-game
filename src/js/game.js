...
export default class Game {
  ...
  newGame = () => {
    ...
    weapons.map((weapon, i) =>
      this.placeItem(
        `<img src="${weapon}" alt="" data-damage="${i > 1 ? i * 10 : 10}" />`,
        "weapon"
      )
    );
  ...
  placeItem = (item, type) => {
    ...
    if (
      !cList.contains("player") &&
      !cList.contains("obstacle") &&
      !cList.contains("weapon")
    ) {
  ...
  //     const move1 = document.querySelector(`[data-row="${direction === 'north' || direction === 'south' ? row operator 1}"][data-col="${col}"]`);
  //     const move2 = element(direction, operator);
  //     const move3 = element(direction, operator);
  ...
  movesHihglights = () => {
      ...
      switch (direction) {
        case "north":
          return document.querySelector(
            `[data-row="${row - num}"][data-col="${col}"]`
          );
        case "south":
          return document.querySelector(
            `[data-row="${Number(row) + num}"][data-col="${col}"]`
          );
        case "west":
          return document.querySelector(
            `[data-row="${row}"][data-col="${Number(col) - num}"]`
          );
        case "east":
          return document.querySelector(
            `[data-row="${row}"][data-col="${Number(col) + num}"]`
    ...
    const availability = (direction) => {
      ...
      if (!move1) return;
      if (
        move1.classList.contains("obstacle") ||
      ...
      if (!move2) return;
      if (
        move2.classList.contains("obstacle") ||
      ...
      if (!move3) return;
      if (
        move3.classList.contains("obstacle") ||
  ...
  movePlayer = (e, move) => {
    ...
    const oldPos = document.querySelector(
      `[data-row="${
        this.players[this.currentPlayer.id - 1].location.row
      }"][data-col="${this.players[this.currentPlayer.id - 1].location.col}"]`
    );
    ...
    for (const elm of document.querySelectorAll("#map > div.highlight")) {
  ...
  detectTurn = () => {
    ...
    console.log(document.querySelector(`#player${this.currentPlayer.id}`));
    document
